...
public class PositionsTest {
	...
	public Object[][] test2() throws Exception {
		Object[][] testData = MyExcelUtil.readData("Test_Data", "PositionsTest");
		return testData;
	...
	public void testMethod_DataSet2(Map<String, String> data) {
		...
		int responseCode = Integer.parseInt(data.get("Http_Response_Code"));
		System.out.println("response.jsonPath().getList(\"$\").size()" + response.jsonPath().getList("$").size());
		Assert.assertEquals(response.statusCode(), responseCode);
		Assert.assertEquals(response.jsonPath().getList("$").size(), Integer.parseInt((data.get("Expected_Data"))));
		// Assert.assertEquals(response.asString().contains(data.get("Http_Response_Message")),
		// true);
	}
	...
	public Object[][] test_DataSet2() throws Exception {
		...
		Map<String, String> interimResult = MyExcelUtil.readDataWithTestCase("Test_Data",
				"Get_Satellite_Positions_Valid_Maximum_Timestamps");
	...
	public Object[][] test_DataSet3() throws Exception {
		Object[][] results = new Object[1][1];
		Map<String, String> interimResult = MyExcelUtil.readDataWithTestCase("Test_Data",
				"Get_Satellite_Positions_Invalid_Maximum_Timestamps");
		results[0] = new Object[] { interimResult };
		return results;
	}
}
